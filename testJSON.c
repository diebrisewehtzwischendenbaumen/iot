...
void main(){
	...
	cJSON *jour = NULL;
	cJSON_ArrayForEach(jour, list)
	{

		i++;
		cJSON *temp = cJSON_GetObjectItemCaseSensitive(jour, "temp");
		cJSON *min = cJSON_GetObjectItemCaseSensitive(temp, "min");
		cJSON *max = cJSON_GetObjectItemCaseSensitive(temp, "max");
		
		printf("- jour %d : min \"%s\" - max \"%s\"\n", i, cJSON_Print(min), cJSON_Print(max));
	}
